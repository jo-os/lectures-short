...
**Заголовок IP-пакета**
- **Поле Номер версии (VERS)** указывает версию протокола IP (4,6).
...
- **Поля Адрес источника (SOURCE IP ADDRESS)**
- **Адрес назначения (DESTINATION IP ADDRESS)** имеют одинаковую длину − по 32 бита и идентичную структуру.
...
**Static NAT** - сопоставление локального IP-адреса с глобальным IP-адресом на основании один к одному, спользуется при необходимости «опубликовать» внутренний сервер компании в интернет, причём не один, а все сервисы сразу. Несмотря на то, что у сервера «серый» адрес, он
...
**Security Association** - базовое понятие IPsec. Включает в себя информацию о криптографических протоколах и алгоритмах, ключах шифрования, определяет какие данные будут проходить через туннель
